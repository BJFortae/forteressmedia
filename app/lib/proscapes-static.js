...
const PROSCAPES_ROOT = join(process.cwd(), "public", "proscapes");
...
function safePath(segments = []) {
  ...
  const filePath = join(PROSCAPES_ROOT, relativePath);
  const rootWithSep = `${PROSCAPES_ROOT}${sep}`;

  if (filePath !== PROSCAPES_ROOT && !filePath.startsWith(rootWithSep)) {
    return null;
  }
